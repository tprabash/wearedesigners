...
class CompanyController extends Controller
{
    ...
    public function store(StoreCompanyRequest $request)
    {
        ...
        if ($request->input('cover_images', false)) {
            $company->addMedia(storage_path('tmp/uploads/' . basename($request->input('cover_images'))))->toMediaCollection('cover_images');
        }
    ...
    public function update(UpdateCompanyRequest $request, Company $company)
    {
        ...
        if ($request->input('cover_images', false)) {
            if (!$company->cover_images || $request->input('cover_images') !== $company->cover_images->file_name) {
                if ($company->cover_images) {
                    $company->cover_images->delete();
                }
                $company->addMedia(storage_path('tmp/uploads/' . basename($request->input('cover_images'))))->toMediaCollection('cover_images');
            }
        } elseif ($company->cover_images) {
            $company->cover_images->delete();
        }
