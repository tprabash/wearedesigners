...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.companies.update", [$company->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="cover_images">{{ trans('cruds.company.fields.cover_images') }}</label>
                <div class="needsclick dropzone {{ $errors->has('cover_images') ? 'is-invalid' : '' }}" id="cover_images-dropzone">
                </div>
                @if($errors->has('cover_images'))
                    <span class="text-danger">{{ $errors->first('cover_images') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.cover_images_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="note">{{ trans('cruds.company.fields.note') }}</label>
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $company->note) }}</textarea>
                @if($errors->has('note'))
                    <span class="text-danger">{{ $errors->first('note') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.company.fields.note_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    Dropzone.options.coverImagesDropzone = {
    url: '{{ route('admin.companies.storeMedia') }}',
    maxFilesize: 2, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    maxFiles: 1,
    ...
    success: function (file, response) {
      $('form').find('input[name="cover_images"]').remove()
      $('form').append('<input type="hidden" name="cover_images" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
      if (file.status !== 'error') {
    ...
    init: function () {
@if(isset($company) && $company->cover_images)
      var file = {!! json_encode($company->cover_images) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="cover_images" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
    error: function (file, response) {
